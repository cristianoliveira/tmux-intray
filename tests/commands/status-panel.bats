...
@test "status-panel shows help" {
    run ./bin/tmux-intray status-panel --help 2>/dev/null
    [ "$status" -eq 0 ]
...
@test "status-panel disabled via --enabled=0" {
    tmux -L "$TMUX_SOCKET_NAME" run-shell "$PWD/bin/tmux-intray add 'Test' 2>&1 >/dev/null"
    run tmux -L "$TMUX_SOCKET_NAME" run-shell "$PWD/bin/tmux-intray status-panel --enabled=0" 2>/dev/null
    [ "$status" -eq 0 ]
    [ -z "$output" ]
}
...
@test "status-panel tmux option updates after dismiss" {
    ...
    id=$(tmux -L "$TMUX_SOCKET_NAME" run-shell "$PWD/bin/tmux-intray add 'Test' 2>&1 | tail -1")
    tmux -L "$TMUX_SOCKET_NAME" run-shell "$PWD/bin/tmux-intray dismiss $id 2>&1 >/dev/null"
    run tmux -L "$TMUX_SOCKET_NAME" show-options -g @tmux_intray_active_count 2>/dev/null
    [ "$status" -eq 0 ]
