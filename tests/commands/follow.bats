...
@test "follow with no notifications exits immediately with timeout" {
    ...
    run timeout 1 tmux -L "$TMUX_SOCKET_NAME" run-shell "echo 'no output expected'; $PWD/tmux-intray follow --interval=0.1" 2>&1
...
@test "follow detects new notification" {
    ...
    (tmux -L "$TMUX_SOCKET_NAME" run-shell "cd $PWD && timeout 2 ./tmux-intray follow --interval=0.1") &
    follow_pid=$!
    sleep 0.5
    # Add a notification
    tmux -L "$TMUX_SOCKET_NAME" run-shell "$PWD/tmux-intray add 'new message'"
    sleep 0.5
...
@test "follow level filter" {
    ...
    tmux -L "$TMUX_SOCKET_NAME" run-shell "$PWD/tmux-intray add --level=info 'info message'"
    ...
    run timeout 1 tmux -L "$TMUX_SOCKET_NAME" run-shell "$PWD/tmux-intray follow --level=error --interval=0.1" 2>&1
    [ "$status" -ne 127 ]
}

@test "follow help" {
    run ./tmux-intray follow --help
    [ "$status" -eq 0 ]
